...
void Java_com_googlecode_tesseract_android_TessBaseAPI_nativeSetImageBitmap(JNIEnv *env,
                                                                             jobject thiz,
                                                                             jobject bitmap) {
  LOGV(__FUNCTION__);
  ...
  if ((ret = AndroidBitmap_getInfo(env, bitmap, &info)) < 0) {
    LOGE("Failed to get Bitmap into! error=%d", ret);
    return;
  }

  if ((ret = AndroidBitmap_lockPixels(env, bitmap, &pixels)) < 0) {
    LOGE("Failed to lock Bitmap pixels! error=%d", ret);
    return;
  }
  ...
  if (bpp > 0) {
    unsigned int size = info.height * info.stride;
    unsigned char *imagedata = (unsigned char *) malloc(size);
    unsigned char *rawpixels = (unsigned char *) pixels;

    // TODO This is probably not entirely accurate for RGB565. Need to re-encode bytes.
    memcpy(rawpixels, imagedata, size);

    native_data_t *nat = get_native_data(env, thiz);
    nat->api.SetImage(imagedata, info.width, info.height, bpp, info.stride);
    ...
    LOGE("Failed to get Bitmap pixel depth!");
  }
...
void Java_com_googlecode_tesseract_android_TessBaseAPI_nativeSetImageBitmap(JNIEnv *env,
                                                                             ...
                                                                             jobject bitmap) {
  LOGV(__FUNCTION__);
  LOGE("Bitmap functions not available; library must be compiled under android-8 NDK");
}
